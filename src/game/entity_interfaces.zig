...
pub const SpriteButtonInterface = struct {
    ...
    pub fn tick(self: *@This(), context: *ECSContext, entity: Entity) void {
        _ = entity;
        var comp_iter = ECSContext.ArchetypeComponentIterator(&.{ TransformComponent, SpriteComponent, ColliderComponent }).init(context);
        while (comp_iter.next()) |iter| {
            const trans_comp = iter.getValue(0);
            const sprite_comp = iter.getValue(1);
            const collider_comp = iter.getValue(2);
